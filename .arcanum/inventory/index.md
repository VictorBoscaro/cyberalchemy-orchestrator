...
- Package is installed but empty: zero sources ingested, so every lookup will
  return a gap until the first `ingest`.
- First ingest target not chosen. Candidates, in the order they would pay off:
  `docs/features/agents-communication-infra/research/` (dense, actively read),
  `docs/decisions/` + `OBLIGATIONS.md` (decision entries), `vault/`
  (contradiction detection against governed meaning).
