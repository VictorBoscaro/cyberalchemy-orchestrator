...
> Defines the enforceable patterns for the **code of the orchestrator engine** — the
> ledger, its appender, the parse/aggregation layer, and the read endpoints that serve
> the control plane. Answers: *"how should any change to the engine's code be built here,
> so the ledger stays an honest, append-only audit trail?"*
>
...
`implementations/server/**` plus the appender that owns the ledger file. It answers one
question: *how must any function that reads, writes, caches, or aggregates the dispatch
ledger be built?*

What we want is a single property: **the ledger is an honest, append-only audit trail, and
no code path corrupts it or lies about it.** Two failures sit at the extremes. A **corrupt
write** poisons the file that every endpoint and aggregation reads from (integrity lost at
the source). A **silent lie** — a count over the served window pretending to be a count over
the whole ledger, a swallowed fault, a computed field overwriting real history — injects
noise the surface then presents as fact.
...
the file is written and honesty where numbers are derived** — the write side protects the
artifact, the read side never invents.
...
- every module under `implementations/server/**` (`ledger.py`, `main.py`, `config.py`) —
  the parse, cache, aggregation, and read-endpoint layer,
- `register-dispatch/append-dispatch.cjs` **as the write contract** — the sole authorized
  writer of the ledger,
- the ledger file itself (`telemetry/agents/subagents-dispatch.yaml`) and the pending
  sheets (`telemetry/agents/pending/*.json`) **as artifacts** — their integrity, not their
  UI form.
...
- touching the appender or anything that could write the ledger file,
...
### EG-1: One Writer — Every Ledger Mutation Goes Through the Validated Appender

The ledger is append-only and owned by `register-dispatch/append-dispatch.cjs`. **No other
code path writes** `subagents-dispatch.yaml`. Every appended record is validated **strictly**
against the current schema — closed enums, required fields, `working_folder` never into
...
- **Falsified if:** a second writer proves genuinely necessary (e.g. a compaction or
  ...
  capability — i.e. single-writer is incompatible with a needed operation.
- **Validation:** `deterministic` — a guard/test asserts no module besides the appender opens
  the ledger for write, and the appender rejects any record failing `validateDispatch`.
  **Blocked** until the drift is traced (see [Promotion Boundary](#promotion-boundary)).
...
- **EG-1 applied to closing a dispatch:** a close is appended by calling the appender with a
  `close_of` record that passes `validateDispatch` — **not** by opening the YAML and writing a
  `close_of:` line by hand. The [[ledger-enum-drift-finding]] is precisely the second path
  happening, which is why EG-1 exists and why its promotion is blocked.
...
- A second script (or an ad-hoc editor keystroke) appending a `close_of:` line straight into
  the YAML, skipping `validateDispatch` (violates EG-1 — this is the drift).
...
- **EG-1 (strict on write) × EG-3 (lenient, degrade never crash):** an apparent tension,
  **reconciled by side** — this is exactly what EG-2 states. Strictness lives at the *write*
  boundary (the appender protects the file); leniency lives at the *read* boundary (the parser
  serves what exists). They never apply to the same operation.
...
- **EG-1:** no guard yet asserts the appender is the *sole* writer, and the drift that
  motivated the rule is not yet traced. This is the one rule with a live counterexample.
...
- If the drift trace shows the ledger *cannot* have a single writer without losing a needed
  operation, EG-1 collapses to "validate at every writer" and the integrity spine weakens —
  the constitution is amended, not retired.
...
- If the engine migrates off the append-only YAML ledger to a database, EG-2/EG-7 (written for
  ...
  survive the migration are EG-1 (one validated writer), EG-4 (honest aggregates), and EG-6
  (history is never re-validated).
...
- if the ledger moves from append-only YAML to a transactional store, the file-shaped rules
