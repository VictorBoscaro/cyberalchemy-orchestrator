...
(async () => {
  ...
  for (const [label, idx] of Object.entries(CASES)) {
    ...
    const url = `http://localhost:${PORT}/variants/carbon.html`;
    try {
      ...
      const diag = await page.evaluate(() => {
        ...
        document.querySelectorAll('.node.group').forEach(g => {
          ...
          groups.push({ label: g.textContent.trim(), clipped: lbl && (lbl.scrollWidth > lbl.clientWidth + 1) });
        });
        return {
          emptyVisible, idleVisible,
          nodes: document.querySelectorAll('#nodes .node').length,
          edges: document.querySelectorAll('#edges .edge').length,
          revNodes: document.querySelectorAll('.node.agent.rev').length,
      ...
      const file = path.join(OUT, `carbon-${label}.png`);
