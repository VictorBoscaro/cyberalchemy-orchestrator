...
function findChrome() {
  ...
  // fall back to puppeteer-downloaded chrome
...
(async () => {
  ...
  for (const [label, idx] of Object.entries(CASES)) {
    ...
    const url = `http://localhost:8107/variants/orbital.html`;
    try {
      ...
      const diag = await page.evaluate(() => {
        ...
        const liveEl = document.getElementById('sh-live');
        const liveVisible = liveEl && getComputedStyle(liveEl).display !== 'none';
        // agent name truncation check: scrollWidth > clientWidth OR scrollHeight overflow
        const agents = [];
        ...
        const rings = document.querySelectorAll('.orbit-ring').length;
        return { emptyVisible, idleVisible, liveVisible, rings,
          anyAgentClipped: agents.some(a => a.clipped),
          agents, groups };
      });
      console.log('===', label, '===');
      console.log(JSON.stringify(diag, null, 1));
      const file = path.join(OUT, `orbital-${label}-after.png`);
