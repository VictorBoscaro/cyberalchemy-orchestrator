...
import { emitTestsPython, emitHybridTestsPython } from "./emit/tests-py.js";
...
function featureDirFor(feature: string): string {
  // tools/test-derivation-engine/src/cli.ts -> repo tools/
  // validation features live in the sibling folder tools/validation/poker-team.
  return resolve(
    __dirname,
...
function runEmitTests(arg: string | undefined): number {
  ...
  // Emitter selected off the sidecar's recorded language (PD-009), never inferred.
  const py = bindings.language === "python";
  const { file, report } = py
    ? emitHybridTestsPython(obligations, bindings, graph)
    : emitHybridTests(obligations, bindings, graph);
  ...
  console.error(
    `coverage_gap:  ${report.coverageGaps} (${py ? "pytest skip" : "it.skip"}, reported not faked)`,
  );
...
/** Emit a structural residue receipt for the selected feature-input boundary. */
function runReceipt(
  arg: string | undefined,
  flags: ReadonlySet<string>,
): number {
  ...
  const rendered = renderResidueReceipt(
    buildResidueReceipt(feature, graph, violations, bindings),
  );
  ...
  const contain = containmentError(outPath, featureDir);
  if (contain) {
    console.error(`receipt: ${contain}`);
    return 6;
  }
  writeFileSync(outPath, rendered, "utf8");
  console.error(`=== receipt: ${feature} ===`);
  console.error(`Wrote: ${outPath}`);
...
function main(): void {
  ...
  switch (command) {
    ...
    case "emit-tests":
      ...
      break;
    case "receipt":
      process.exit(runReceipt(arg, flags));
      break;
    case "lint":
    ...
    default:
      console.error(
        `unknown command: ${command ?? "(none)"} — expected roundtrip | self-check | derive [--out] | check | emit-tests | receipt [--out] | lint`,
      );
      process.exit(1);
  }
}
