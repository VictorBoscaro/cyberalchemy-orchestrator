...
/** Target test language for a feature. Explicit per-feature (PD-009: stack is
 *  scoped) — the CLI selects the vitest vs pytest emitter off this field, so the
 *  language is a recorded decision in the sidecar, never inferred from a filename. */
export type TestLanguage = "typescript" | "python";

export interface BindingSet {
  readonly feature: string;
  readonly emit_dir: string;
  readonly test_file: string;
  /** Emitter language. Absent/omitted is treated as "typescript" so pre-existing
   *  sidecars and hand-built sets are unchanged; loadBindings always resolves it. */
...
export function loadBindings(path: string): BindingSet {
  ...
  return {
    ...
    language: raw.language === "python" ? "python" : "typescript",
