...
- deterministic Session, Dispatch, Research and per-agent reference projections at an explicit
  event offset.
...
| SC-ACI-05 | [ACI target-agent reference delivery](../../agents-communication-infra/specs/domain.md#agentreferencedelivery) | yes | Owns accepted target Attempt/seat/agent-instance, bundle membership and exact effective-input entry; specified, not implemented. |
| SC-HOST-01 | [Mandatory host wrapper](../integration/stage-f/mandatory-host-wrapper.md#host-wiring) | yes | Owns observable dispatch ingestion and preserves `exact | metadata_only | opaque` coverage without semantic upgrading. |
| SC-HOST-02 | [AgentReferenceLineage producer-resolution source contract](queries.md#agentreferencelineage) | yes | Requires a separately pinned `host.AgentActivationBinding` wrapper that resolves stored capture producers bijectively to accepted Attempt/agent-instance identities; specified, not implemented. |
...
| Goal | Query the path from recommendation to one owner-resolved target without collapsing evidence axes. | Operators need per-attempt answers while delivery, observation, declaration and claim evidence retain distinct meanings. |
...
|---|---|---|
| Human/project owner `@victor` | Solely owns mutation-gate elevation with an independent receipt; has no implicit runtime rollover privilege. | [Work-pack gate authority](../WORK-PACK.md#construction-integration-and-enablement-gates) |
...
| ACI target-delivery authority | Owns `AgentReferenceDelivery`, target Attempt identities and the exact `EffectiveInputArtifact.reference_bundle` entry. | [ACI target delivery](../../agents-communication-infra/specs/domain.md#agentreferencedelivery) |
...
graph LR
    ...
    HSO --> EB
    ARD[ACI AgentReferenceDelivery / EffectiveInput owner] --> EB
    EB --> APP
...
| Agent-reference binder | [Agent Reference Lineage](SPEC.md#capabilities) | Resolve `attempt | seat | agent_instance` to exact owner-stamped Attempt identities, derive the canonical capture-producer selector, then pin the host producer-resolution, accepted ACI delivery/effective-input and host observation evidence before pure reduction. |
...
| Domain validators | APT-R1 through APT-R9 form/invariant predicates | Schema and already-bound evidence values only | Pure, deterministic, side-effect free and unable to resolve external evidence. |
...
| Agent-reference query service | `AgentReferenceLineage` binding and projection | ACI accepted-prefix/delivery reader, host activation-binding owner, host observation owner and APT fact reducer | Target delivery and declared-use attribution use independently pinned owner IDs and exact evidence only; locator equality is never a join key. The contract is specified, not implemented. |
...
For `AgentReferenceLineage`, the read half binds the selector to an exact canonical Attempt set,
verifies ACI delivery/effective-input, derives the canonical selector for eligible stored
`ResearchCapture.producer_ref` values, binds that selector bijectively through the separately
pinned `host.AgentActivationBinding` owner wrapper, and verifies host observations through their
owner. It passes only immutable pinned values to the reducer. The reducer emits independent
evidence collections and performs no owner call. This complete read contract is specified but not
implemented; the current pilot still yields `access_observed=[]`.
...
| Read agent reference lineage | Resolve `attempt | seat | agent_instance` through ACI ownership, bind accepted target delivery/effective input, derive the capture-producer selector, bind its complete `host.AgentActivationBinding` resolution and host observation facts, then reduce at one verified boundary. | Ambiguous/foreign identity, producer-selector/member non-bijection, locator-only match, coverage upgrade, future/dangling evidence or incomplete group fails closed; absence on one axis never fills another. The query remains specified/not implemented. | [Reference separation](../discovery/session-dispatch-research-records.md#43-reference-uses-and-checks) |
...
| Agent-reference axis | recommended / delivered / access observed / declared use / claim relation / claim-support check | Each is derived only from its owning accepted evidence; no positive axis implies another. | Six independent collections/statuses, never one `used` boolean. |
...
| ACI target-delivery/effective-input reader | outbound | [AgentReferenceDelivery](../../agents-communication-infra/specs/domain.md#agentreferencedelivery) and [EffectiveInputArtifact](../../agents-communication-infra/specs/domain.md#effectiveinputartifact) | Reads accepted owner facts only; Attempt/seat/agent-instance are owner-stamped, and locator matching is forbidden. |
...
| Host agent-activation binding owner interface | outbound external dependency | [AgentReferenceLineage producer resolution](queries.md#agentreferencelineage) | Returns one complete versioned/scope-bound member per canonical capture-producer selector and no extra member; the binder pins the wrapper before pure reduction. APT neither defines owner truth nor infers `agent_instance_id`. |
...
| APT-AR13 | `AgentReferenceLineage` target delivery and host joins require the complete ACI owner-resolved tuple `(dispatch_id,group_id,seat_id,attempt_id,agent_instance_id)` plus the exact accepted `AgentReferenceDelivery` and recommendation identities; locators are display evidence, never keys. A stored `ResearchCapture.producer_ref.(dispatch_id,group_id,seat_id,attempt_id,activation_id)` can join only through the separately pinned, bijective `host.AgentActivationBinding` resolution and cannot itself supply `agent_instance_id`. | Agent-reference binder and reducer | Cross-identity, missing/extra/duplicate/future/cross-scope producer resolution, ambiguous selector, locator collision and dangling-evidence tests. |
| APT-AR14 | `recommended`, `delivered`, `access_observed`, `declared_used`, `claim_relation` and `claim_support_check` remain independent; host coverage/action are preserved exactly and cannot be upgraded. | Agent-reference reducer/read adapter | All asymmetric presence combinations, coverage downgrade/upgrade and claim-check typing tests. |
...
| ACI `AgentReferenceDelivery` and `EffectiveInputArtifact` refs | ACI runtime/input-settlement authority | Exact target delivery and observable input inclusion, not access or use. | [ACI target-delivery event](../../agents-communication-infra/specs/events.md#referencescoutbundledeliveredtoagent) |
| Pinned `host.AgentActivationBinding` / `producer_resolution` wrapper | External host activation-binding authority | Resolve every canonical `capture_producer_selector` bijectively to an accepted activation/Attempt/agent-instance tuple before deriving `declared_used`; specified, not implemented. | [HostAgentActivationBindingEvidencePort](interfaces.md#hostagentactivationbindingevidenceport) |
...
| Gate review receipt | Independent reviewer/project owner workflow | Prove documentation/readiness conditions; not runtime state. | [Mutation gate](../WORK-PACK.md#construction-integration-and-enablement-gates) |
...
| Read-time cross-owner lineage join | Avoids a second persisted lineage authority or migration. | Query binding must pin more external evidence. | Owner-resolved selector, complete manifest/digests, exact IDs and pure reducer. |
...
| APT-ARCH-D3 | Add AgentReferenceLineage as a read-time projection rather than persist an APT copy of target delivery. | Persist delivery pin/event vs join accepted owner facts at query bind time. | ACI already owns target delivery/effective input; a second record would create migration and authority ambiguity. |
...
| APT-RK12 | A locator match or Scout-worker observation is misattributed as target-agent access/use. | Exact owner-derived dispatch/seat/attempt joins, independent axes and negative asymmetric fixtures. | Query/domain owners |
| APT-RK13 | An implementation accepts a partial, stale or non-bijective producer-resolution wrapper and thereby misattributes declared use to a target agent. | Require the complete accepted Query wrapper: exact host owner/version, five-field upstream-digest scope, accepted boundary, non-self-referential owner digest, pinned wrapper digest and one activation/Attempt member per canonical selector with no extras. | Query/architecture owners |
...
- Status: `flag`
- Reason: the aspect corpus, storage/artifact contracts and TEST-SPEC exist; the
  AgentReferenceLineage contract now includes the separately pinned producer owner-resolution
  evidence required for deterministic `declared_used` attribution, but the query and its host
  owner contracts are specified/not implemented. TEST-SPEC remains `planned/not-run`; the
  corpus-wide review receipt, profile digests, executable evidence and implementation-readiness
  verdict remain outstanding.
- Required follow-up: close corpus-wide objections, freeze the exact ACI profile evidence, implement
  the versioned owner binders and query/reducer, execute
