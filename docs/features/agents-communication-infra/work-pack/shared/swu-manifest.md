...
| SWU-ACI-CONFIRMATION-AUTHORITY-000 | TASK-CONF-000 | reviewed `confirmed-dispatch-v1` golden and independent CONF-000 validation | none; historical test-only proposal closed without implementation | reviewed canonical package, negative corpus, failpoint inventory and independent PASS | final manifest and CONF-000 evidence pins | `CLOSED`; `SUPERSEDED_BY_REVIEWED_GOLDEN` |
| SWU-ACI-CONFIRMED-DISPATCH-001 | TASK-CONF-001 | closed CONF-000 golden + alignment reconciliation + layering PASS + exact readiness receipt | exact 14-path ordered descriptor scope: migration 012, metadata-aware authorized artifact read, pure confirmation kernel, specialized journal writer, service, focused tests/traceability and Stage-E integrity refresh | one atomic accepted unit at durable `opening_pending`; 56 negatives; 21 failpoints; replay/conflict/concurrency/reopen; zero external action | focused confirmation, Stage-B, traceability, Stage-C, bridge, full runtime, compileall and diff check | one coder, then independent verifier; no API/UI/effect claim/continuation |
