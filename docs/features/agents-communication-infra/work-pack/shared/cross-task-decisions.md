...
| D-111 | Owner selects only `SWU-ACI-APT-VS-001` for mutation-test implementation after independent Stage-A review cycle 5/5. | `workPackGateStatus=pass-for-exact-swu`; `mutationTestAuthorization=pass_for_exact_swu`; local serve and every production/cutover gate remain blocked pending implementation evidence. |
| D-112 | Owner proposes local-pilot serving for the independently verified `SWU-ACI-APT-VS-001` implementation. | Operative only after independent review of the final Stage-B receipt and this decision; limited to explicit `127.0.0.1`, a dedicated DB, the configured strict ledger, and no external network, provider execution, agent launch, materializer or cutover. |
...
| D-CVR-009 | Bind the three-owner acceptance set, policy, repository, audience, executor and finalizer in authorization; use one immutable create-exclusive claim lease and an ephemeral external `AuthorityLaunchContext`. | Exactly three artifacts remain persisted; hashes prove integrity only, while authenticated identity/time/nonces remain externally supplied prerequisites. |
