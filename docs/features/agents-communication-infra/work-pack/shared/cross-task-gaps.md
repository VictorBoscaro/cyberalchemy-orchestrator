...
| G-PG-001: protocol-compilation readiness pins are stale | `SWU-ACI-PROTOCOL-COMPILATION-001` coder entry | closed | Descriptor, context-index and TEST-SPEC hashes match the reissued `domainspec-code-readiness@1` PASS receipt. |
| G-PG-002: PC1–PC12 execution evidence is pending | Protocol Governance adjunct completion | closed for bounded SWU | Nine-path closure, 13 focused tests, 131 runtime tests and independent verifier PASS are recorded; broader conformance remains out of scope. |
| G-PG-003: obsolete `ACI-030` could be mistaken for this compiler | ownership and layer traceability | repaired in planning | Use only `SWU-ACI-PROTOCOL-COMPILATION-001`/`TASK-PROTOCOL-COMPILATION`; retain `ACI-030` solely as the historical W6/L4 placeholder. |
