...
| `assignments[].node_id` | trusted allocator evidence | compiler exact-coverage gate | joins one allocator assignment to exactly one DraftGraph node |
| `assignments[].display_name` | trusted allocator evidence | compiler pool-membership/non-reuse/role-fit gate | selects the final identity without granting DraftGraph authority over names |
| `assignments[].role_fit_override` | trusted allocator evidence | compiler override-policy gate | carries explicit evidence when selected identity does not list the node role |
