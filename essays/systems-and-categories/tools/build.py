...
ESSAY_ROOT = Path(__file__).resolve().parents[1]
REPO_ROOT = Path(__file__).resolve().parents[3]
MD = ESSAY_ROOT / "pdf/systems-and-categories.md"
OUT = ESSAY_ROOT / "pdf/systems-and-categories.pdf"
MEDIA = ESSAY_ROOT / "drawings"
FONTS = Path(__file__).resolve().parent / "fonts"
RENDER_DIR = REPO_ROOT / "tmp/pdfs/systems-and-categories"
...
def cover(story):
    ...
    img=Image(str(MEDIA/"cover.png"),width=430,height=240); story += [img,Spacer(1,105),
...
def parse_md():
    ...
    while i<len(lines):
        ...
        if raw.startswith("<!--"):
            ...
            elif "embedded image 2" in txt: story += [Image(str(MEDIA/"systems-lens.png"),width=430,height=126)]
            elif "embedded image 3" in txt: story += [Image(str(MEDIA/"category-lens.png"),width=430,height=161)]
...
def check():
    ...
    for number, page in enumerate(reader.pages, 1):
        ...
        if abs(width - A4[0]) > 1 or abs(height - A4[1]) > 1:
            raise RuntimeError(f"Page {number} is not A4")
    text = "\n".join((page.extract_text() or "") for page in reader.pages)
    for expected in ("SYSTEMS AND CATEGORIES", "Toward Domain Languages", "Victor Boscaro"):
        if expected not in text:
            raise RuntimeError(f"Missing expected text: {expected}")
    print(f"OK: {len(reader.pages)} A4 pages with expected text")


def render():
    executable = shutil.which("pdftoppm")
    if not executable:
        raise RuntimeError("pdftoppm is required to render previews")
    RENDER_DIR.mkdir(parents=True, exist_ok=True)
    subprocess.run(
        [executable, "-png", "-r", "160", str(OUT), str(RENDER_DIR / "page")],
        check=True,
    )
    print(RENDER_DIR)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build and inspect the essay PDF.")
    parser.add_argument("command", nargs="?", choices=("build", "check", "render", "all"), default="all")
