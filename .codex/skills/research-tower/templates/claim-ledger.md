# Source Claim Ledger

Target: `<paper/source/topic>`

| ID  | Claim     | Source kind      | Evidence                  | Local reading        | Status    |
| --- | --------- | ---------------- | ------------------------- | -------------------- | --------- | ---- | -------- |
| C1  | `<claim>` | `primary-source` | `<section/page/url/path>` | `<operator reading>` | `accepted | open | blocked` |

## Source-Kind Rules

- `primary-source`: stated by the main source.
- `related-source`: stated by a related paper or external source.
- `local-inference`: inferred from source-backed material.
- `analogy`: useful Arcanum mapping, not source meaning.
- `operator-reading`: practical design interpretation.
- `open-residue`: unresolved question or future task.

## Open Claim Residue

| ID  | Question     | Blocking artifact | Next route            |
| --- | ------------ | ----------------- | --------------------- |
| R1  | `<question>` | `<artifact>`      | `<task/sigil/source>` |
