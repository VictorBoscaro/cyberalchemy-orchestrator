...
> **Ported to `domainspec-core`.** The ontology this skill invokes (`node_type`, the
> `## Connections` edge catalog) and the `vault/discovery/` promotion target are
> canonical in the `implementation/domainspec` submodule — vault paths below resolve
> against `implementation/domainspec/vault/`. See Navigation at the bottom.
...
- **Recommended next action** — promote as a discovery (knowledge scope: `implementation/domainspec/vault/discovery/<topic>-definitions/<slug>.md`; application scope: `docs/features/<feature>/discovery/<slug>.md`), re-dispatch with adjusted strategy, or hold.
...
- Frontmatter cheatsheet: `implementation/domainspec/.claude/skills/custom/frontmatter.md`
- Edge catalog: `implementation/domainspec/.claude/skills/custom/edges.md`
- How to write the upstream research file: `./research-writing.md`
- Node-type semantics: `implementation/domainspec/.claude/skills/custom/frontmatter-semantics.md`
